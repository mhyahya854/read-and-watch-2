...
Current Phase 17 evidence (2026-09-23): the application's resolved external benchmark root contains 16 registered formal candidates from 10 source hashes (4 English, 4 Arabic, 6 Urdu, 2 English/Arabic mixed; 14 PAGE, 2 LINE, 0 REGION). All 16 ground truths remain DRAFT, none is locked or FINAL, and no formal provider run exists. Seven drafts were bootstrapped from native text and still need independent human review; nine are marked for manual transcription. Existing source and sample-image hashes match, but the candidates do not contain an explicit lawful-use declaration. The corpus does not yet establish representative acceptance coverage. The host has Intel Arc graphics and no NVIDIA CUDA device; the resolved external root has no activated OCR runtime/model. PP-OCRv5's pinned CPU combination and the specialist's short runtime-root repair remain the accepted, previously proven configuration; their older failures below are historical, not current product blockers. No numerical or sufficiently explicit qualitative P17-G002 acceptance decision rule is documented. P17-T004, P17-T007, and P17-G002 remain open; Phase 18 remains NOT_STARTED.
