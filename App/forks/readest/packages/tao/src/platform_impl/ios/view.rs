...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // Implementing this selector stops UIKit consulting the Info.plist
      // UIApplicationSceneManifest, so it has to answer for EVERY session role,
      // not just the window scene. Hardcoding
      // UIWindowSceneSessionRoleApplication handed a connecting CarPlay session
      // a TaoSceneDelegate, which implements none of the CarPlay lifecycle
      // methods, so -[CPTemplateApplicationScene
      // _deliverInterfaceControllerToDelegate] raised NSGenericException
      // ("Application does not implement CarPlay template application lifecycle
      // methods in its scene delegate") and killed the app on connect.
      let role_ptr: id = msg_send![connecting_scene_session, role];
      let role: &NSString = &*(role_ptr as *const NSString);

      if role.to_string() != "UIWindowSceneSessionRoleApplication" {
        // Non-window roles (CarPlay, external display, ...) are declared by the
        // app in its Info.plist. A nil name makes UIKit resolve the
        // configuration - and with it the delegate class - from the manifest
        // entry for this role, which is what it would have done had this
        // selector been absent.
        let config = UISceneConfiguration::configurationWithName_sessionRole(None, role, mtm);
        return Retained::autorelease_return(config) as _;
      }
      ...
      // Return the configuration at +0 via objc_autoreleaseReturnValue.
      // `Retained::as_ptr(&config)` returned a raw borrow of an object that
      // was dropped (released) when this function returned, so UIKit's
      // `-[UIApplication _connectUISceneFromFBSScene:transitionContext:]`
      // retained a freed object - a use-after-free that crashed at launch
      // (SIGSEGV in objc_retain) on real devices whenever the app has a
      // UIApplicationSceneManifest (e.g. a CarPlay scene declaration).
