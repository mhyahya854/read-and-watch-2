...
export function userDataPlugin({ userDataRoot, catalogPath }) {
  const store = createUserDataStore({ userDataRoot, catalogPath });

  return {
    name: 'local-user-data',
    configureServer(server) {
      server.middlewares.use('/api/user-data', async (request, response, next) => {
        try {
          ...
          // /index
          if (parts.length === 1 && parts[0] === 'index' && request.method === 'GET') {
            return sendJson(response, 200, store.getIndex());
          }
          // /:type/:itemId
          if (parts.length !== 2 || !['thoughts', 'notes'].includes(parts[0])) {
            return sendJson(response, 400, { error: 'Invalid user-data route' });
          }
          ...
          if (request.method === 'PUT') {
            ...
            const result = store.save(type, itemId, body.content, body.baseRevision ?? null);
          ...
          return sendJson(response, 400, { error: error?.message ?? 'User-data error' });
        }
      });
    },
  };
