/**
 * Runtime SQLite schema bootstrap.
 *
 * The canonical migration files live under `import/migrations/`. The packaged
 * desktop runtime does not ship the `import/` tree, so this module carries a
 * byte-identical embedded snapshot of the forward-only migrations and a test
 * compares every template against the canonical file.
 *
 * `applyRuntimeSchema` is idempotent:
 *   - a fresh database receives migration 001 and then migration 002
 *   - an existing Phase 02 database receives migration 002 only
 *   - a current database is left untouched
 */
import { createHash } from 'node:crypto';

export const RUNTIME_SCHEMA_VERSION = 2;

const INITIAL_TEMPLATE = `BEGIN IMMEDIATE;
...
const RELAX_IDENTITY_TEMPLATE = `BEGIN IMMEDIATE;
...
function withChecksum(template) {
  return template.replace('__CHECKSUM__', checksum(template));
}
...
export function applyRuntimeSchema(database) {
  database.exec('PRAGMA foreign_keys = ON');

  let created = false;
  if (!tableExists(database, 'items')) {
    database.exec(withChecksum(INITIAL_TEMPLATE));
    created = true;
  }
  if (!tableExists(database, 'schema_migrations')) {
    throw new Error(
      'The runtime database has an items table but no schema_migrations table; refusing to guess its schema.',
    );
  }
  ...
  if (version < 2) {
    ...
    try {
      database.exec(withChecksum(RELAX_IDENTITY_TEMPLATE));
    } finally {
...
export const RUNTIME_SCHEMA_TEMPLATES = Object.freeze({
  initial: INITIAL_TEMPLATE,
  relaxItemIdentity: RELAX_IDENTITY_TEMPLATE,
});
